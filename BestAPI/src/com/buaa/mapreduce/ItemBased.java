...
public class ItemBased {
	...
	public static class ItemBasedMapper 
	...
	public static class ItemBasedReducer
...
//		}
		...
		protected void reduce(Text key, Iterable<Text> values,
			...
			int[] itopk = findApiTopK(b);
			
			double p = 0;
			double q = 0;
			for(int i = 0; i < k; ++i) {
				if(b[b.length-i-2] > 0 && a[itopk[i]] != 0 && a[itopk[i]] != -1) {
					p += b[b.length-i-2] * a[itopk[i]];
					q += b[b.length-i-2];
				}
			}
			double r = 0.0;
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
	    ...
	    job.setJarByClass(ItemBased.class);
	    job.setMapperClass(ItemBasedMapper.class);
//	    job.setCombinerClass(ItemBasedReducer.class);
	    job.setReducerClass(ItemBasedReducer.class);
